<div style="width:403px;clear:both;margin:10px auto 20px;">
                ...
                $('#<?php echo $form['id'];?>').ajaxForm({
                    ...
                    complete: function(data) {
                        ...
                        if(data.responseJSON.status=='ok')
                            window.location='<?php echo site_url();?>suppliers/edit/'+data.responseJSON.supplier_id;
                        else
                        {
                            ...
                            if(data.responseJSON.status=='no_name')
                            {
                                ...
                                $('#msgholder-supplier_name').html('').html('Please give a name!').slideDown();
                            }
                            else if(data.responseJSON.status=='no_code')
                            {
                                $('#msgholder-supplier_code').parent().css('height','auto');
                                $('#msgholder-supplier_code').html('').html('Please give a code!').slideDown();
                            }
                            else if(data.responseJSON.status=='invalid_code')
                            {
                                $('#msgholder-supplier_code').parent().css('height','auto');
                                $('#msgholder-supplier_code').html('').html('Invalid code!').slideDown();
                            }
                            else if(data.responseJSON.status=='already_exists')
                            {
                                $('#msgholder-supplier_code').parent().css('height','auto');
                                $('#msgholder-supplier_code').html('').html('supplier with this code already exists!').slideDown();
                            }
                            else if(data.responseJSON.status=='unknown_error')
                            {
                                $('#msgholder-supplier_code').parent().css('height','auto');
                                $('#msgholder-supplier_code').html('').html('An unknown error occured! Please contact technical support').slideDown();
                            }
                        }
                    }
                });
