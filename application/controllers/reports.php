...
class Reports extends CI_Controller
{
    ...
    public function viewPdf()
    {
        if (user_logged_in() && user_can('GENERATE_REPORT')) {
            ...
            if ($this->input->post('active_1') == true) {
                $this->db->where('products.department', $this->input->post('filter_1'));
            }
            if ($this->input->post('active_2') == true) {
                $this->db->where('stocks.sid', $this->input->post('filter_2'));
            }
            if ($this->input->post('from') != false) {
                $this->db->where('stocks.stocked_on >=', $this->input->post('from'));
            }
            if ($this->input->post('to') != false) {
                $this->db->where('stocks.stocked_on <=', $this->input->post('to'));
            }
            ...
            if ($this->input->post('sort_by') == 'sid') {
                $this->db->order_by('suppliers.name', $this->input->post('order'));
            } elseif ($this->input->post('sort_by') == 'department') {
                $this->db->order_by('departments.name', $this->input->post('order'));
            } else {
                $this->db->order_by($this->input->post('sort_by'), $this->input->post('order'));
            }
            ...
            $data['total_total_price'] = 0;
                ...
                if (sizeof($data['results'] > 0)) {
                    ...
                    if ($this->input->post('active_1')) {
                        $data['file'] .= '_Dep_'.preg_replace('/[^a-zA-Z0-9]+/', '_', $data['results'][0]['department']);
                    }
                    if ($this->input->post('active_2')) {
                        $data['file'] .= '_Sup_'.preg_replace('/[^a-zA-Z0-9]+/', '_', $data['results'][0]['supplier']);
                    }
                    if ($this->input->post('include_cost') != 1) {
                        $data['file'] .= '_without_cost';
                    }
    ...
    public function viewPdf2()
    {
        if (user_logged_in() && user_can('GENERATE_REPORT')) {
            //$data['active_1']=$this->input->post('active_1');
                $data['active_2'] = $this->input->post('active_2');//department
                $data['active_3'] = $this->input->post('active_3');//supplier
                //$data['filter_1']=$this->input->post('filter_1');
                $data['filter_2'] = $this->input->post('filter_2');
            $data['filter_3'] = $this->input->post('filter_3');
            $data['sort_by'] = $this->input->post('sort_by');
            $data['order'] = $this->input->post('order');
            $data['from'] = $this->input->post('from');//date
                $data['to'] = $this->input->post('to');//date
                //header('Content-type: application/json');
                //echo json_encode($data);
                //die();
                     $this->db->select('*');
            $this->db->from('invoices');
                     ...
                     if ($this->input->post('active_2')) {
                         $this->db->where('products.department', $this->input->post('filter_2'));
                     }
            if ($this->input->post('active_3')) {
                $this->db->where('stocks.sid', $this->input->post('filter_3'));
            }
            if ($this->input->post('from')) {
                $this->db->where('invoices.bill_time >=', $this->input->post('from'));
            }
            if ($this->input->post('to')) {
                $this->db->where('invoices.bill_time <=', $this->input->post('to'));
            }
            ...
            $data['total_total_sale'] = $total_total->sale;
                ...
                if (sizeof($data['results'] > 0)) {
                    ...
                    $div = $body->appendChild($div);
                    $div->setAttribute('style', 'font-size: 30px;font-family: monospace;text-align:center;');
                    $text = $doc->createTextNode('Sell Info Report');
                    $text = $div->appendChild($text);
                    ...
                    if ($this->input->post('active_3') == true) {
                        $div = $doc->createElement('div');
                        $div = $body->appendChild($div);
                        $div->setAttribute('style', 'font-size: 15px;font-family: monospace;text-align:left;float:left;clear:left;margin-left:5%;');
                        $text = $doc->createTextNode('Supplier : '.$data['results'][0]['supplier']);
                    ...
                    $table = $body->appendChild($table);
                          ...
                          //invoices.generated_id as invoice, orders.order_id as oid, barcode,sku,products.name as name,departments.name as department, suppliers.name as supplier, (stocks.unit_cost * orders.quantity) as total_cost,, (stocks.unit_sale * orders.quantity) as total_sale, orders.quantity as quantity, invoices.bill_time as time

                          $display_keys = array(
                              'name' => array('label' => 'Product Name', 'style' => ''),
                              'sku' => array('label' => 'SKU', 'style' => ''),
                              'barcode' => array('label' => 'Barcode', 'style' => ''),
                              //'unit_cost'=>array('label'=>'Unit Cost','style'=>'text-align:right;'),
                              'quantity' => array('label' => 'Quantity', 'style' => 'text-align:right;'),
                              'unit_sale' => array('label' => 'Sales Price', 'style' => 'text-align:right;'),
                              //'total_cost'=>array('label'=>'Total Cost','style'=>'text-align:right;'),
                              'total_sale' => array('label' => 'Total', 'style' => 'text-align:right;'),
                              //'stocked_on'=>array('label'=>'Entry Time','style'=>'text-align:right;')
                              );
                    if ($this->input->post('include_cost') == 1) {
                        $display_keys = array(
                    ...
                    if ($this->input->post('include_cost') == 1) {
                        ...
                        $div = $body->appendChild($div);
                        $div->setAttribute('style', 'font-size: 15px;font-family: monospace;text-align:right;margin-right:5%;');
                        $text = $doc->createTextNode('Total Price : '.$data['total_total_sale']);
                        $text = $div->appendChild($text);
                    } else {
                        $div = $doc->createElement('div');
                        $div = $body->appendChild($div);
    ...
    public function fetch()
    {
        if (user_logged_in() && user_can('GENERATE_REPORT') && $this->input->post('type') == 'stockentry') {
        ...
        } elseif (user_logged_in() && user_can('GENERATE_REPORT') && $this->input->post('type') == 'combined_sellinfo') {
            header('Content-type: application/json');
                ...
                /* Search */
                    if (in_array($this->input->get_post('filter_by_1'), array('sku', 'name')) && in_array($this->input->get_post('filter_by_2'), array('did')) && in_array($this->input->get_post('filter_by_3'), array('sid'))) {
                        ...
                        if (sizeof($sales) > 0) {
                            if ((int) $this->input->post('active_1') == 1) {
                                if ($this->input->post('filter_1')) {
                                    ...
                                    } elseif ($this->input->post('filter_by_1') == 'name') {
                        ...
                        if (sizeof($sales) > 0) {
                            if ($order_by == 'asc') {
                                if ($sort_by == 'name') {
                                    usort($sales, function ($a, $b) {return strcmp($a['name'], $b['name']);});
                                } elseif ($sort_by == 'sku') {
                                    usort($sales, function ($a, $b) {return strcmp($a['sku'], $b['sku']);});
                                } elseif ($sort_by == 'department') {
                                    usort($sales, function ($a, $b) {return strcmp($a['department'], $b['department']);});
                                } elseif ($sort_by == 'supplier') {
                                    usort($sales, function ($a, $b) {return strcmp($a['supplier'], $b['supplier']);});
                                } elseif ($sort_by == 'total_cost') {
                                    usort($sales, function ($a, $b) {return $a['total_cost'] - $b['total_cost'];});
                                } elseif ($sort_by == 'total_sale') {
                                    usort($sales, function ($a, $b) {return $a['total_sale'] - $b['total_sale'];});
                                } elseif ($sort_by == 'quantity') {
                                    usort($sales, function ($a, $b) {return $a['quantity'] - $b['quantity'];});
                                }
                            } else {
                                if ($sort_by == 'name') {
                                    usort($sales, function ($a, $b) {return strcmp($b['name'], $a['name']);});
                                } elseif ($sort_by == 'sku') {
                                    usort($sales, function ($a, $b) {return strcmp($b['sku'], $a['sku']);});
                                } elseif ($sort_by == 'department') {
                                    usort($sales, function ($a, $b) {return strcmp($b['department'], $a['department']);});
                                } elseif ($sort_by == 'supplier') {
                                    usort($sales, function ($a, $b) {return strcmp($b['supplier'], $a['supplier']);});
                                } elseif ($sort_by == 'total_cost') {
                                    usort($sales, function ($a, $b) {return $b['total_cost'] - $a['total_cost'];});
                                } elseif ($sort_by == 'total_sale') {
                                    usort($sales, function ($a, $b) {return $b['total_sale'] - $a['total_sale'];});
                                } elseif ($sort_by == 'quantity') {
                                    usort($sales, function ($a, $b) {return $b['quantity'] - $a['quantity'];});
                                }
                            }
        ...
        } elseif (user_logged_in() && user_can('GENERATE_REPORT') && $this->input->post('type') == 'sellinfo') {
            header('Content-type: application/json');
            ...
            if (in_array($sort_by, array('name', 'sku', 'did', 'sid', 'total_cost', 'total_sale', 'quantity', 'time'))) {
                /* Search */
                    if (in_array($this->input->get_post('filter_by_1'), array('sku', 'barcode', 'name')) && in_array($this->input->get_post('filter_by_2'), array('did')) && in_array($this->input->get_post('filter_by_3'), array('sid'))) {
                        ...
                        $this->db->from('invoices');
                        if ((int) $this->input->post('active_1') == 1) {
                            $this->db->like('products.'.$this->input->post('filter_by_1'), $this->input->post('filter_1'), 'both');
                        }
                        if ((int) $this->input->post('active_2') == 1) {
                            $this->db->where('products.department', $this->input->post('filter_2'));
                        }
                        ...
                        if ($sort_by == 'time') {
                            $this->db->order_by('invoices.bill_time', $order_by);
                        }
                        if ($sort_by == 'total_cost') {
                            $this->db->order_by('total_cost', $order_by);
                        }
                        if ($sort_by == 'total_sale') {
                            $this->db->order_by('total_sale', $order_by);
                        }
                        if ($sort_by == 'sid') {
                            $this->db->order_by('supplier', $order_by);
                        }
                        if ($sort_by == 'did') {
                            $this->db->order_by('department', $order_by);
                        }
                        if ($sort_by == 'quantity') {
                            $this->db->order_by('quantity', $order_by);
                        }
                        if ($sort_by == 'barcode') {
                            $this->db->order_by('barcode', $order_by);
                        }
                        if ($sort_by == 'sku') {
                            $this->db->order_by('sku', $order_by);
                        }
                        if ($sort_by == 'name') {
        ...
        } elseif (user_logged_in() && user_can('GENERATE_REPORT') && $this->input->post('type') == 'profit') {
            header('Content-type: application/json');
            ...
            echo json_encode($data);
            die();
        } elseif (user_logged_in() && user_can('GENERATE_REPORT') && $this->input->post('type') == 'currentstock') {
            header('Content-type: application/json');
