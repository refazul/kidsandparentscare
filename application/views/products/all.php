    ...
    <div class="sort_by-wrapper">        
        <div class="select-wrap">
            <select name="sort_by" id="sort_by" onchange="$('#products-fetch').submit();">
                <?php foreach($fields as $key=>$value): if($key=='stock' || $key=='price')continue;?>
                ...
                complete: function(data) {
                    ...
                    if(data.responseJSON.status=='ok')
                    {
                        ...
                        if(data.responseJSON.results.length>0)
                        {
                            ...
                            <?php foreach($fields as $key=>$value):if(!isset($value[2]))$value[2]='right;padding-right:10px';?>
                            ...
                            for(i=0;i<results.length;i++)
                            {
                                ...
                                $('<td/>',{style:'text-align:right;padding-right:10px;'}).append(document.createTextNode(results[i].stock)).appendTo('#product-'+results[i].pid);
                                $('<td/>',{style:'text-align:right;padding-right:10px;'}).append(document.createTextNode(results[i].price)).appendTo('#product-'+results[i].pid);
