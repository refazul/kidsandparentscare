	...
	public function index()
	{
        ...
        public function create()
        {
            if(user_logged_in() && user_can('CREATE_USER'))
            {
                unset($temp);
                $this->db->where('rid >','1');
                $temp=$this->db->get('roles')->result_array();
                foreach($temp as $key=>$value)
                    $user['roles'][$value['rid']]=$value['role'];
                
                $this->load->view('header');
                $this->load->view('menu');
                $this->load->view('wrap_begin');
                $this->load->view('users/create',$user);
        ...
        {
            if(user_logged_in() && user_can('EDIT_USER'))
            {
        ...
        public function fetch()
        {
            if(user_logged_in())
            {
                ...
                if(in_array($sort_by, $this->db->list_fields('users')))
                {
                    ...
                    $this->db->order_by($sort_by, $order);
                    $data['results']=$this->db->get('users',$limit,$limit*$page)->result_array();                    
                    $data['page']=$page;
                    $data['limit']=$limit;
                    $data['status']='ok';
                    ...
                    {
                        $this->db->where('rid',$data['results'][$key]['rid']);                        
                        $data['results'][$key]['role']=$this->db->get('roles')->row(0,'object')->role;                        
                    }
                }
                else                
            ...
            if($this->input->get_post('intent'))
            {
                ...
                {                    
                    if($this->input->post('user_name') && $this->input->post('user_pass'))
                    {
                        ...
                        {                            
                            if(!valid_role($this->input->post('user_role')))
                            {
                                $json['status']='invalid_role';
                                echo json_encode($json);
                                die();
                            }
                            $data['user']=$this->input->post('user_name');
                            $data['pass']=$this->input->post('user_pass');
                            $data['rid']=$this->input->post('user_role');
                            ...
                            $data['image']=$this->input->post('user_image')?$this->input->post('user_image'):'';
                ...
                else if($intent='edit' && user_logged_in() && user_can('EDIT_USER'))
                {
