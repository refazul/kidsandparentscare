	...
	public function index()
	{
        ...
        public function fetch()
        {
            if(user_logged_in())
            {
                if($this->input->post('invoice'))
                {
                    ...
                    if(in_array($this->input->post('filter_by'),array('barcode','name','sku','unit')))$this->db->like($this->input->post('filter_by'),$this->input->post('filter'),'both');
                    else if(in_array($this->input->post('filter_by'),array('department')))$this->db->where($this->input->post('filter_by'),$this->input->post('filter'));
        ...
        public function miniedit()
        {
            if(user_logged_in() && user_can('EDIT_PRODUCT'))
            {
                if($this->uri->segment(3) && valid_integer($this->uri->segment(3)))
                {
                    ...
                    if(isset($preload))$product['preload']=$preload;
        ...
        public function ajax()
        {
            ...
            if($this->input->get_post('intent'))
            {
                ...
                else if($intent=='edit' && user_logged_in() && user_can('EDIT_PRODUCT'))
                {
                ...
                else if($intent=='search' && user_logged_in() && user_can('CREATE_INVOICE'))
                {
                    header('Content-type: application/json');
                    $sort_by=$this->input->get_post('sort_by') ? $this->input->get_post('sort_by') : 'pid';
                    $order=$this->input->get_post('order') ? $this->input->get_post('order') : 'asc';
